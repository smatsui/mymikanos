...
void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
  ...
  uint32_t ehci2xhci_ports = pci::ReadConfReg(xhc_dev, 0xd3);
...
extern "C" void KernelMainNewStack(const FrameBufferConfig& frame_buffer_config_ref,
  ...
  console = new(console_buf) Console{kDesktopFGColor,
                                     kDesktopBGColor};
  console->SetWriter(pixel_writer);
  printk("Welcome to my MikanOS!\n");
  SetLogLevel(kWarn);
  ...
  auto mouse_window = std::make_shared<Window>(kMouseCursorWidth,
                                               kMouseCursorHeight);
